...
exports.handler = async () => {
  ...
  const res = await fetch(`${SUPA_URL}/rest/v1/bell_preferences?lauds=eq.true&select=user_id,timezone`,
  ...
  for (const user of eligible) {
    await fetch('https://onesignal.com/api/v1/notifications', {
      ...
      body: JSON.stringify({ app_id: ONESIGNAL_APP_ID, include_aliases: { external_id: [user.user_id] }, target_channel: 'push', headings: { en: 'Lauds' }, contents: { en: 'The bell rings for Lauds. Be still and enter prayer.' }, url: 'https://stillprayer.app' })
    });
  }
